...
class Command(BaseCommand):
    help = 'Sets up demo data including company and sample calendar events'
    ...
    def handle(self, *args, **options):
        try:
            ...
            company, company_created = Company.objects.get_or_create(
                name="Demo Company",
                defaults={
                    'address': "123 Demo Street, Demo City, 12345, Demo Country",
            ...
            if not profile_created and not profile.company:
                profile.company = company
                profile.save()
                self.stdout.write(self.style.SUCCESS(f'Updated profile for user: {user.username}'))
            elif profile_created:
                self.stdout.write(self.style.SUCCESS(f'Created profile for user: {user.username}'))
            else:
                self.stdout.write(self.style.SUCCESS('User profile already exists with company'))
